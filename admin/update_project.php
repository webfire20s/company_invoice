...
<div class="flex h-screen overflow-hidden">
    ...
    <aside class="w-72 bg-slate-950 text-white hidden md:flex flex-col border-r border-slate-800">
        ...
        <nav class="flex-1 px-4 space-y-1 overflow-y-auto no-scrollbar">
            ...
            <a href="projects_list.php"
                class="flex items-center space-x-3 <?php echo (in_array(basename($_SERVER['PHP_SELF']), ['projects_list.php', 'update_project.php'])) ? 'bg-blue-600 text-white shadow-lg shadow-blue-900/20' : 'text-slate-400 hover:bg-slate-900 hover:text-white'; ?> px-4 py-3 rounded-xl transition-all duration-200">
    ...
    <main class="flex-1 overflow-y-auto">
        <header class="bg-white border-b border-slate-200 px-10 py-6 sticky top-0 z-20">
            <div class="flex items-center space-x-4">
                <a href="projects_list.php" class="p-2 hover:bg-slate-100 rounded-full transition-colors text-slate-400">
                    <i data-lucide="arrow-left" class="w-5 h-5"></i>
                </a>
                <div>
                    <h2 class="text-2xl font-bold text-slate-800">Update Project Progress</h2>
                    <p class="text-xs text-slate-500 font-medium uppercase tracking-wider">Milestone Tracking / Project ID: <?= $project['id'] ?></p>
                </div>
            </div>
        </header>

        <div class="p-10 flex justify-center items-start pt-20">
            <div class="form-card w-full max-w-md rounded-3xl shadow-xl overflow-hidden p-8">
                <form method="POST" class="space-y-8 text-center">
                    ...
                    <div>
                        <div class="mb-6 flex flex-col items-center">
                            ...
                            <h3 class="text-lg font-bold text-slate-700"><?= $project['project_name'] ?></h3>
                        </div>

                        <label class="block text-[11px] font-bold uppercase tracking-widest text-slate-500 mb-6">Completion Percentage</label>
                        
                        <div class="relative pt-1">
                            <input type="number" name="progress" id="progressInput" value="<?= $project['progress'] ?>" min="0" max="100"
                                   class="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-4 text-2xl font-bold text-center text-blue-600 input-focus outline-none mb-2">
                            <p class="text-[10px] text-slate-400 font-medium italic">Enter value from 0 to 100</p>
                        </div>
                    </div>
